...
describe('filterCollectionByKeyValueTags', () => {
  ...
  it('filters string tags by $eq', async () => {
    ...
    const result = await filterCollectionByKeyValueTags({
      andConditions: [{ product: { $eq: 'Product A' } }],
      orConditions: [],
      teamId: 'team-1',
      datasetIds: ['ds-1']
    });
  ...
  it('filters datetime tags by $lt', async () => {
    ...
    const result = await filterCollectionByKeyValueTags({
      andConditions: [{ date: { $lt: 1704153600000 } }],
      orConditions: [],
      teamId: 'team-1',
      datasetIds: ['ds-1']
    });
  ...
  it('handles AND + OR combination', async () => {
    ...
    const result = await filterCollectionByKeyValueTags({
      andConditions: [{ product: { $eq: 'Product A' } }, { version: { $gte: 2 } }],
      orConditions: [{ category: { $eq: 'warranty' } }],
      teamId: 'team-1',
      datasetIds: ['ds-1']
    });
  ...
  it('fails when OR tag does not exist', async () => {
    ...
    const result = await filterCollectionByKeyValueTags({
      andConditions: [{ product: { $eq: 'A' } }],
      orConditions: [{ missing: { $eq: 'x' } }],
      teamId: 'team-1',
      datasetIds: ['ds-1']
    });
  ...
  it('$empty/$notEmpty only match existing tag entries', async () => {
    ...
    const emptyResult = await filterCollectionByKeyValueTags({
      andConditions: [{ product: { $empty: true } }],
      orConditions: [],
      teamId: 'team-1',
      datasetIds: ['ds-1']
    });
    ...
    const notEmptyResult = await filterCollectionByKeyValueTags({
      andConditions: [{ product: { $notEmpty: true } }],
      orConditions: [],
      teamId: 'team-1',
      datasetIds: ['ds-1']
    });
...
describe('filterCollectionByMetadata', () => {
  ...
  it('keeps old format behavior unchanged', async () => {
    mockMongoDatasetCollectionTagsFind.mockReturnValue({
      lean: vi.fn().mockResolvedValue([{ _id: 'tag-1', datasetId: 'ds-1', tag: 'Tag1' }])
    });
    mockMongoDatasetCollectionFind.mockReturnValue(mockFind([{ _id: 'col-1' }]));
